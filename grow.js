...
(function () {
    ...
    /** @type {!Object.<string, string>} */ const resource_root =
        RESOURCE_PATHS["Grow"];
    ...
    var genRate = 0.4; /* 1 is full, 0 is none */
    ...
    var lavaCoolSpeed = 0.01; /* cool on solids */
    var lavaFireSpeed = 0.05; /* spawn fire rate */

    // var wallMeltLavaSpeed = .08;
    var wallMeltFireSpeed = 0.03;

    var cloudDropletSpeed = 0.15;
    var cloudLightningSpeed = 0.00003;

    var shrubBarkVertSpeed = 0.02;
    var shrubBarkHoriSpeed = 0.5;
    var shrubGrowBarkSpeed = 0.6;
    var shrubVertSpeed = 0.4;
    var shrubHoriSpeed = 0.6;
    var shrubHoriDist = 4;
    var shrubGrowthSpeed = 0.3;
    var shrubAgeSpeed = 0.05;
    /*  /////// // End Variables // ///////  */
    ...
    function reset() {
        ...
        particles = [];
        modPart = new Array(
            parseInt(window.innerWidth / particleSize + 1, 10) *
                parseInt(window.innerHeight / particleSize + 1, 10),
        );
        modInd = 0;
        updatedInd = 0;
        updatedPart = new Array(
            parseInt(window.innerWidth / particleSize + 1, 10),
        );
        for (
            var i = 0;
            i < parseInt(window.innerWidth / particleSize + 1, 10);
            i++
        ) {
            updatedPart[i] = new Array(
                parseInt(window.innerHeight / particleSize + 1, 10),
            );
            particles.push(
                new Array(parseInt(window.innerHeight / particleSize + 1, 10)),
            );
            for (
                var j = 0;
                j < parseInt(window.innerHeight / particleSize + 1, 10);
                j++
            ) {
    ...
    function getBeneath(event) {
        ...
        var starter = document.elementFromPoint(
    ...
    function init() {
        ...
        body.addEventListener(
            ...
            function (event) {
                if (isClear(event)) {
                    try {
                        getBeneath(event).dispatchEvent(clickevent);
                    } catch (e) {}
                }
                mouseMove(event);
            },
        ...
        body.addEventListener(
            ...
            function (event) {
                ...
                if (ben != null && ben.nodeName == "A")
                    canvas.style.cursor = "pointer";
                else if (ben != null) canvas.style.cursor = ben.style.cursor;
                else canvas.style.cursor = "default";
                if (isDown) mouseMove(event);
            },
            false,
        );
        body.addEventListener(
        ...
        point.fillText(
            loadingText,
            (window.innerWidth - point.measureText(loadingText).width) / 2,
            ...
            while (txt != "" && done) {
                ...
                if (tagType == "elem") {
                    while (txt.indexOf(">") != 0) {
                        ...
                        if (property == "shape") shape = txt.substring(0, next);
                        else if (property == "x")
                ...
                if (tagType == "elem") {
                    ...
                    point.clearRect(
                        player.x * particleSize,
                        player.y * particleSize,
                        particleSize,
                        particleSize,
                    );
                    if (positioning == "relative") {
    ...
    function onKeyDown(e) {
        ...
        else if (code == 37) /* Arrow Keys */ player.move = -1;
        else if (code == 39) player.move = 1;
        else if (code == 38)
            if (player.jump == 0 && !player.hasJumped) {
                player.hasJumped = true;
                player.jump = 5;
            }
        if (code == 17 && showPlayer)
            /* CTRL key */
            ...
            alert(
                "updatedPart: " +
                    updatedPart[parseInt(curX / particleSize, 10)][
                        parseInt(curY / particleSize, 10)
                    ] +
                    "\npariticles: " +
                    particles[parseInt(curX / particleSize, 10)][
                        parseInt(curY / particleSize, 10)
                    ].fresh,
            );
            ...
            for (var i = -1; i < 2; i++)
                for (var j = -1; j < 2; j++)
                    if (
                        x + i < 0 ||
                        x + i > particles.length ||
                        y + j < 0 ||
                        y + j > particles[0].length ||
                        isSolid(particles[x + i][y + j].fresh)
                    )
                        count++;
            alert(count);
        } else if (code == 32) /* Space */ player.fireBullet();
        else if (code == 48) /* 0 */ penSize = defaultPenSize;
        else if (code >= 49 && code <= 57) /* 1 - 9 */ penSize = code - 48;
        else if (code == 65) elemType = "sand";
        else if (code == 66) elemType = "bark";
        else if (code == 67) elemType = "cloud";
        else if (code == 68) elemType = "droplet";
        else if (code == 69) elemType = "eraser";
        else if (code == 70) elemType = "fire";
        else if (code == 71) elemType = "growth";
        else if (code == 72) elemType = "shrub";
        else if (code == 73) elemType = "ice";
        else if (code == 75) elemType = "cloak";
        else if (code == 76) elemType = "lightning";
        else if (code == 77) elemType = "smoke";
        else if (code == 78) elemType = "nut";
        else if (code == 79) elemType = "soil";
        else if (code == 80) elemType = "plague";
        else if (code == 83) elemType = "steam";
        else if (code == 84) elemType = "sticky";
        else if (code == 86) elemType = "lava";
        else if (code == 87) elemType = "wall";
        else if (code == 88) elemType = "explosive";
        else if (code == 90) elemType = "liquidExplosive";
        else if (code == 192) {
            /*~*/
        ...
        this.fireBullet = function () {
            for (var i = 0; i < numBullets; i++) {
                ...
                if (
                    bullet.x < 0 ||
                    bullet.y < 0 ||
                    bullet.x > particles.length ||
                    bullet.y > particles[0].length
                ) {
                    ...
                    if (magnitude > maxBulletSpeed) magnitude /= maxBulletSpeed;
                    else magnitude = 1;
    ...
    function touchMove(evt) {
        try {
            ...
            for (var i = 0; i < touches.length; i++) {
                circlePoint(
                    parseInt(touches[i].clientX / particleSize, 10),
                    parseInt(touches[i].clientY / particleSize, 10),
                    penSize,
                    getElem(),
                    true,
                    [],
    ...
    function mouseMove(event) {
        if (event.clientX != 0 || event.clientY != 0) {
            if (playerMode) {
                ...
                do {
                    circlePoint(
                        parseInt(x / particleSize, 10),
                        parseInt(y / particleSize, 10),
                        penSize,
                        getElem(),
                        true,
                        [],
    ...
    function move() {
        ...
        if (magnitude > shootDist) magnitude /= shootDist;
        else magnitude = 1;
    ...
    function getElem() {
        if (gen && elemType != "plague" && !isSolid(elemType))
            return elemType + "Gen";
        else if (elemType == "eraser") return "empty";
    ...
    function updateAction(x, y) {
        ...
        if (
            ...
            if (move == 0) changePoint(x, y - 1, "plague");
            else if (move == 1) changePoint(x, y + 1, "plague");
            else if (move == 2) changePoint(x - 1, y, "plague");
            else if (move == 3) changePoint(x + 1, y, "plague");

            if (Math.random() < 0.25) changePoint(x, y, "empty");
        } else if (elem == "steam") {
            var move = Math.floor(Math.random() * 100);
            if (move >= 60) changePoint(x, y - 1, "steam");
            else if (move >= 50) changePoint(x - 1, y, "steam");
            else if (move >= 40) changePoint(x + 1, y, "steam");
            else if (move == 1) changePoint(x, y, "droplet");

            if (move >= 40) changePoint(x, y, "empty");
        } else if (elem == "cloud") {
            var x2 = -1,
                y2 = -1;
            ...
            if (move < 20) {
                ...
                /* 25 & 4 // 278 & 5 //6 is too tight, makes a square */
            ...
            if (x2 != -1) {
                ...
                for (var i = -1; i < 2; i++)
                    for (var j = -1; j < 2; j++)
                        try {
                            if (particles[x + i][y + j].fresh == "cloud") {
                                sum++;
                            }
                        } catch (e) {}
                if (sum > 4) {
            ...
            if (spawn < cloudLightningSpeed) changePoint(x, y + 1, "lightning");
                    ...
                    for (var j = -1; j < 2; j++)
                        try {
                            if (
                                ((i == 0 || j == 0) &&
                                    isSolid(particles[x + i][y + j].fresh)) ||
                                (isSticky(particles[x + i][y + j].fresh) &&
                                    (!isLiquid(particles[x + i][y + j].fresh) ||
                                        isClumpy(
                                            particles[x + i][y + j].fresh,
                                        )))
                            ) {
                                continue;
                            }
            ...
            if (elem == "lava") {
                ...
                if (Math.random() < lavaFireSpeed) changePoint(x, y, "fire");
            } else {
                changePoint(x, y, "empty");
            ...
            if (move % 3 == 0) changePoint(x, y - 1, "fire");
            if (move < 1) changePoint(x, y - 3, "smoke");
            if (move > 6) changePoint(x + 1, y, "fire");
            if (move < 2) changePoint(x - 1, y, "fire");
            if (move == 0) changePoint(x, y - 1, "fire");
            changePoint(x, y, "empty");
        } else if (elem == "smoke") {
            var move = Math.floor(Math.random() * 9);
            var amount = Math.floor(Math.random() * 5) - 2;
            if (move > 7) changePoint(x, y - Math.abs(amount), "smoke");
            if (move < 6) changePoint(x, y, "empty");
            if (move > 6) changePoint(x + amount, y, "smoke");
            if (move < 2) changePoint(x - amount, y, "smoke");
        } else if (elem == "shrub") {
            var grow = Math.random();
            if (
                grow < shrubVertSpeed &&
                y - 1 >= 0 &&
                particles[x][y - 1].fresh == "empty"
            )
            ...
            if (grow < shrubHoriSpeed) {
                ...
                try {
                    if (grow > 0.5) {
                        while (
                            dist++ < maxDist &&
                            particles[x + dist][y].fresh == "empty"
                        ) {
                            ...
                            if (
                                Math.random() < 0.5 &&
                                particles[x + dist][y - 1].fresh == "empty"
                            )
                                changePoint(x + dist, y - 1, "bark");
                        }
                    } else {
                        while (
                            ...
                            if (particles[x - dist][y + 1].fresh == "empty")
                                changePoint(x - dist, y + 1, "bark");
                            if (
                                Math.random() < shrubGrowBarkSpeed &&
                                particles[x - dist][y - 1].fresh == "empty"
                            )
                                changePoint(x - dist, y - 1, "bark");
                        }
                ...
                for (var j = -1; j < 2; j++) {
                    try {
                        ...
                        if (
                            ...
                            if (
                                (i + j) % 2 == 0 &&
                                particles[x + i][y + j].fresh == "empty"
                            )
                                if (
                                    j != 0 &&
                                    Math.random() < shrubBarkVertSpeed
                                )
                                    changePoint(x + i, y + j, "bark");
                                else if (Math.random() < shrubBarkHoriSpeed)
                                    changePoint(x + i, y + j, "bark");
                                else if (
                                    Math.random() < shrubGrowthSpeed &&
                                    particles[x + i][y + j].fresh == "empty"
                                )
            ...
            isGen(elem) &&
            isGas(
                particulate.fresh.substring(
                    0,
                    particulate.fresh.indexOf("Gen"),
                ),
            )
        ) {
            changePoint(
    ...
    function draw() {
        ...
        if (showPlayer) {
            ...
            for (var i = 0; i < numBullets; i++) {
                point.fillRect(
                    player.bullets[i].x * particleSize,
                    player.bullets[i].y * particleSize,
                    particleSize,
                    particleSize,
                );
            }
    ...
    function updatePlayer() {
        ...
        for (var i = 0; i < numBullets; i++) {
            var bullet = player.bullets[i];
            point.clearRect(
                bullet.x * particleSize,
                bullet.y * particleSize,
                particleSize,
                particleSize,
            );
            ...
            try {
                if (
                    isSolid(particles[x][y].fresh) ||
                    particles[x][y].fresh == "sticky" ||
                    getDensity(particles[x][y].fresh) > getDensity("droplet")
                ) {
                    circlePoint(
                        x,
                        y,
                        ...
                        bullet.elem,
                        true,
                        ["growth", "explosive", "liquidExplosive", "cloak"],
                    );
                    player.bullets[i] = new Bullet();
                } else {
                    ...
                    if (
                        isSolid(particles[x][y].fresh) ||
                        particles[x][y].fresh == "sticky" ||
                        getDensity(particles[x][y].fresh) >
                            getDensity("droplet")
                    ) {
                        circlePoint(
                            x,
    ...
    function changePoint(x, y, type) {
        try {
            ...
            particle.old = particle.fresh;
            particle.fresh = type;
            if (particle.old == particle.fresh) forcePoint(x, y, type);
            ...
            if (type == "droplet") {
                for (var i = -1; i < 2; i++)
                    for (var j = -1; j < 2; j++)
                        if (
                            particles[x + i][y + j].fresh == "growth" &&
                            Math.random() < growthSpeed
                        ) {
                            setTimeout(
                                changePoint,
                                gameSpeed * 3,
                            ...
                            particles[x + i][y + j].fresh == "ice" &&
                            Math.random() < iceSpeed
                        ) {
                ...
                if (particle.old == "fire") {
                    ...
                    if (
                        (particle.fresh == "growth" &&
                            Math.random() < growthSpeed) ||
                        (particle.fresh == "ice" && Math.random() < iceSpeed)
                    ) {
            ...
            if (type == "fire") {
                if (isCombustible(particle.old)) {
                    setTimeout(circlePoint, gameSpeed, x, y, 10, "fire", true, [
                        "explosive",
                        "liquidExplosive",
                        "droplet",
                    ]);
                ...
                for (var i = -1; i < 2; i++)
                    for (var j = -1; j < 2; j++)
                        /* if (particles[x + i][y + j].fresh=='wall' && Math.random()<wallMeltLavaSpeed) {
                            ...
                            isFlammable(particles[x + i][y + j].fresh) &&
                            Math.random() < lavaSpeed
                        ) {
                            setTimeout(
                                changePoint,
                                gameSpeed * 3,
            ...
            if (type == "lightning") {
                if (isCombustible(particle.old) || isSolid(particle.old))
                    ...
                    particle.old != "empty" &&
                    particle.old != "lightning" &&
                    particle.old != "droplet" &&
                    particle.old != "fire" &&
                    !isGas(particle.old)
                )
                    forcePoint(x, y, particle.old);
            ...
            if (isSolid(particle.old) && isLiquid(type)) {
                ...
                if (particles[x - 1][y - 1].fresh == "empty")
                    ...
                    else flow = Math.floor(Math.random() * 2) - 1;
                else if (particles[x + 1][y - 1].fresh == "empty")
            ...
            if (isLiquid(particle.old) && isLiquid(type)) {
                if (getDensity(particle.old) >= getDensity(type)) {
                    /* e.g. droplet falling onto soil */
                    forcePoint(x, y, particle.old);
                    var flow = Math.floor(Math.random() * 4);
                    if (
                        flow <= 1 &&
                        ((isLiquid(particles[x - 1][y - 1].fresh) &&
                            getDensity(particles[x - 1][y - 1].fresh) <
                                getDensity(particle.old)) ||
                            particles[x - 1][y - 1].fresh == "empty")
                    )
    ...
    function elipsePoint(x, y, xr, yr, type, force, exceptions) {
        try {
            for (var i = 0; i < xr; i++) {
                for (var j = 0; j < yr; j++) {
                    var dist = Math.sqrt(
                        Math.pow(i / xr, 2) + Math.pow(j / yr, 2),
                    );
                    if (dist <= 1) {
                        if (force) {
                            if (exceptions.indexOf(type) != -1) {
                                try {
                                    changePoint(x + i, y + j, type);
                                } catch (e) {}
                                try {
                                    changePoint(x - i, y + j, type);
                                } catch (e) {}
